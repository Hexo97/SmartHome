...
export default function ManageSensors() {
  ...
  return (
    ...
    <View style= {{ backgroundColor:"#4DA8DA", height:50, margin:5, marginBottom:10}}>
        <Text style= {{ color: 'black',textAlign:"center",marginTop:10, fontSize:18 }}>Whole App Sensor's Simulator</Text>
        </View>
  ...
  <View style={{margin:10}}>
      ...
      <ListItem style={{backgroundColor:"#EEFBFB",alignItems: "center",
      marginHorizontal:30}}>
      <ListItem.Content>
      <Text style={{fontSize:20,fontWeight:"bold"}}>Status {out?.status} </Text>
      {
          out?.status
          &&
          out?.status == "Running"
      ...
      <ListItem.Content>
        {
          out?.delay
          ?
          <Text style={{fontSize:20,fontWeight:"bold"}}>Delay {out?.delay} </Text>
          :
